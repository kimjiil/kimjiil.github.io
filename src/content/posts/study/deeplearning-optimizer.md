...
bias correction인 $\hat{v}_{t}$와 $\hat{m}_{t}$은 $m_{t}, v_{t}$가 특정 방향으로 편향되지 않도록 방향을 올바르게 바로 잡아주기 위한
...
여기서 $\zeta$값은 매우 작게 유지 되거나 0이다. $v_{t}$의 평균값을 거의 gradient의 평균값에 근접하도록 하려면 $(1-\beta_{2}^{t})$을 나눠주면 되는데 
